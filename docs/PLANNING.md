...
This is the map of Pulse's planning documents and the order they are produced in. Pulse is a multi-tenant SaaS uptime monitoring platform (distributed Go services, Postgres + Redis + a pluggable event bus, Kubernetes, Lit SPA). The bus is selected by `PULSE_BUS`: Kafka (default, via Redpanda) or Redis Streams. The planning is layered on purpose: product first (PRDs), then technical design (RFCs), then decisions (ADRs), then the delivery plan.
...
6. **Phased delivery** - the phase plan lives in `PRD.md` section 15 (phases, scope per phase, done-when), not a separate roadmap file.
...
- PRD-006 Billing & Entitlements - plans, tiers, seats, per-API metering, quota enforcement, Paddle billing - DONE
- PRD-007 Multi-Region - region selection, per-region results, quorum down-policy, probe-fleet health, region cost - DONE
- Cross-PRD consistency review - DONE (6 mechanical fixes applied, conflicts resolved to defaults)

Lead decisions from the review (locked): 3-day trial (7 on annual) (PRD-006 canonical), 14-day org-deletion grace, incident-duration-based uptime (PRD-002 canonical). Remaining minor nits (glossary check/probe, create-org row in master matrix, PRD-004 uptime citation) folded into the final doc-polish pass; not blocking.
...
- Cross-RFC consistency review - DONE / RESOLVED (5 mechanical fixes applied, the flagged seams C-1/C-2/C-3/G-1/G-2/G-3 resolved by lead decision and applied across the docs)
...
- ADR-0002 PostgreSQL via pgx + goose migrations - DONE
...
- ADR-0017 Frontend stack (Lit light DOM + Tailwind + uPlot + TanStack Table) - DONE. Current state: daisyUI was dropped for an owned Swiss design token + primitive layer; Tailwind, Lit, uPlot, and TanStack Table stay.
...
7. Phased delivery plan (PRD.md section 15).
...
Build has started (ahead of ADRs and the phase plan, by choice). Done so far: the multi-service foundation (five services + `cmd/schema`, Postgres + Redis + pluggable bus wiring, RLS tenancy); the scheduler -> worker -> alerting -> notifier check pipeline reusing `internal/checker`; HTTP and SSL-expiry monitor types (SSL cert-expiry shipped, cert_expires_at in the API); billing on Paddle (subscriptions, payments, the 3-day trial with a re-trial deny window); nine notification channels (slack, discord, webhook, smtp, telegram, pagerduty, opsgenie, teams, twilio); the observability stack (Loki, Tempo, Prometheus, Grafana with SLO dashboards, Alertmanager) under `observability/`; and the web Swiss redesign (owned tokens, daisyUI dropped). The v1 spine (SQLite store, single-admin auth, embedded web, `cmd/pulse`) has been removed; the leaf packages carry forward as planned. See `README.md` for detail.
