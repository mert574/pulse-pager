...
Phase: enforcement model lands in Phase 1; Stripe and metered billing land in Phase 2 (master section 15). The entitlement model and enforcement are needed early because they gate core behavior.
...
- **Stripe** (payment, plan management, invoices, proration, subscription-state webhooks) is Phase 2. Until Stripe is wired, plans are set internally by an operator and limits are still enforced exactly the same way (master section 11: "Until then, plans are set internally and limits are still enforced").
...
- **Building a billing/payment engine.** Stripe owns money movement, plan catalog pricing, proration math, dunning, and invoices (section 8 of this doc). Pulse owns entitlement enforcement only.
...
- In Phase 2: `stripe_customer_id`, `stripe_subscription_id`, `current_period_end`.
...
A billing record owned by Stripe. Pulse stores a lightweight reference (Stripe invoice id, amount, period, status, hosted PDF URL) for display in the billing UI (master section 10 screen 12). Pulse does not compute invoice line items; it reads them from Stripe. Before Phase 2 there are no invoices (Free and internally-set plans only).
...
Organization 1..* Invoice (Phase 2, mirrored from Stripe)
...
| **Channel types** | All v1 (Slack/Discord/webhook/email) | All v1 | All + PagerDuty/Opsgenie (phased) | All |
...
- Channel-type differences track the phased channel roadmap (master section 7, section 15 Phase 3); v1 channels are available on every tier.
...
|--------------|---------------------|-----------------------|
| Create monitor | `monitors_cap` (enabled count) | Reject with `code: monitor_limit_reached`, upgrade prompt. On Free, the 3rd monitor (the one over the cap of 2) is blocked. |
...
The scheduler owns the schedule for all orgs' monitors (master section 6.6) and must independently respect the org's interval floor and region entitlement when it dispatches checks. This is the no-bypass guarantee: a monitor created under Business at a 1-minute interval across 6 regions cannot keep running that way after the org downgrades to Free, even though the stored monitor config still says 60 seconds and 6 regions.
...
An upgrade takes effect immediately. Higher caps, faster interval floor, more regions, more seats, longer retention, and richer features become available as soon as the new plan is recorded and the entitlement cache is invalidated (section 5.3). In Phase 2, Stripe collects payment and proration for the remainder of the cycle is Stripe-handled (section 8). In Phase 1, an operator sets the plan internally and the upgrade is immediate with no payment.
...
The billing UI presents this as a checklist ("to switch to Starter, bring usage under these limits") and the downgrade button stays disabled until the org is within the target plan. This is testable (section 10).

### 6.3 Proration

Proration is **Stripe-handled and phased** (Phase 2, master section 11). When a customer changes plan or seat count mid-cycle, Stripe computes the prorated charge or credit for the remainder of the billing period; Pulse does not compute proration itself. Before Phase 2 there is no proration because there is no payment (Free and internally-set plans only).
...
- **Frequency tier**: the current min-interval floor (e.g. "checks as fast as every 1 min" on Team).
...
- **Invoices** (Phase 2): list of Stripe invoices with amount, period, status, hosted PDF link. Owner manages payment method; admin views.
...
## 8. Stripe integration (phased, Phase 2)

### 8.1 What Stripe owns

- **Payment**: card capture, PCI scope, charging.
- **Plan management / catalog pricing**: prices for the base package, the per-seat add-on, and the per-monitor add-on live as Stripe prices; plan changes go through Stripe.
...
- **Subscription-state webhooks**: Stripe is the source of truth for subscription status; it notifies Pulse via webhook on `created` / `updated` / `payment_failed` / `canceled` / `period renewed`, and Pulse updates its Subscription record and invalidates the entitlement cache (section 5.3).

### 8.2 What Pulse owns

- **Entitlement enforcement.** This is Pulse's job, always, regardless of Stripe. Stripe says what plan the org is on; Pulse turns that into the entitlement set and enforces it at the api and scheduler (section 5). Stripe never enforces a monitor cap or an interval floor; that is product behavior in Pulse.
...
### 8.3 Before Stripe (Phase 1)

Plans are set internally by an operator. Free is the default at signup (master section 3). Limits are enforced identically; the only thing missing is payment, invoices, and proration. This is the master's explicit position (section 11): the model and enforcement do not wait for Stripe.
...
- **Trial recommendation**: offer a **14-day trial of the Team tier, no card required up front**, started from the billing screen. During the trial the org's entitlement is Team; on trial end without payment, the org drops to Free and the downgrade rules (section 6.2) apply (the owner is prompted to bring usage under Free before the drop completes, so nothing is silently deleted). 14 days with no card maximizes activation for the primary revenue persona (master persona B, Team) while the no-silent-delete rule protects their data. This is an open decision with this as the recommended default (section 11).
...
- As **platform operations (Phase 1)**, I want to set an org's plan internally and have limits enforced immediately, before Stripe exists.

### 10.2 Acceptance criteria (testable)

1. **3rd monitor on Free is blocked.** Free cap is 2. Creating a 3rd enabled monitor on a Free org is rejected with `code: monitor_limit_reached` and an upgrade prompt; the monitor is not created. (The locked anchor is 2; the 3rd is the one over the cap.)
...
4. **Downgrade requires bringing usage under the limit.** An org with 10 enabled monitors trying to downgrade to Free (cap 2) cannot complete the downgrade until enabled monitors are <= 2; the downgrade action stays disabled and lists what to trim (section 6.2). No monitor is deleted automatically.
...
6. **Scheduler refuses a de-entitled region after downgrade.** A monitor configured for 6 regions on an org downgraded to a 1-region plan is dispatched only to the home region; the other 5 region jobs are not enqueued (section 5.2).
...
3. **Annual vs monthly billing.** Recommended: **offer both, default the pricing page to monthly, with an annual option at a discount (around two months free).** Monthly lowers the entry barrier for the price-sensitive Dev persona; annual improves retention and cash for the Team persona. Phase 2 (Stripe handles both cycles). `billing_cycle` is on the Subscription (section 2.3).
4. **Trial length and card requirement.** Recommended: **14-day Team trial, no card up front**, dropping to Free on expiry under the no-silent-delete rule (section 8.4). Trade-off: no-card trials attract some tire-kickers, but the activation lift for the primary revenue persona is worth it, and the Free fallback is safe.
