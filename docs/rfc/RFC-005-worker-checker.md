...
The worker emits `check.results` to its regional bus and does not write the durable `check_results` row itself (it does write the `monitor_last_failure` and `monitor_cert` side tables, section 5.4). A small control-plane consumer (`result-persister`, in the `pulse-control` namespace next to Postgres) consumes the mirrored `check.results`, performs the idempotent upsert from section 5.2, and that is the durable write. Alerting consumes the same `check.results` stream independently for the verdict.
...
The worker does not write the durable `check_results` row; alerting owns that upsert. The worker does touch Postgres for two small side tables, `UpsertMonitorLastFailure` and `UpsertMonitorCert` (`internal/worker/worker.go`), and uses Redis for live check-state. RFC-006 owns the alerting transaction that performs the idempotent result upsert; this RFC fixes only that the worker emits and does not persist the `check_results` row, and that the durable write is the `(org_id, monitor_id, region, checked_at)` upsert from RFC-001 section 7.4 wherever it runs.
...
| Control | Requirement |
|---------|-------------|
| NetworkPolicy egress | worker pods may egress only to: the regional bus (Kafka or Redis Streams), Postgres and Redis (for the side-table writes and live check-state from section 5), the public internet for checks, DNS, and the metrics/observability sinks. No egress to other internal service ranges |
...
PRD master section 12 sets ~10k checks/sec sustained as the single-region baseline; multiplied by the per-monitor region count (Free 1 up to Custom 4, PRD-006 / PRD-007) the platform-wide check execution rate is order 10k-40k/sec (RFC-002 section 9.1). Per region, the worker fleet must drain its region's share. With 64 partitions per region (RFC-002 section 3.3) and in-pod concurrency `W` (section 3.4), one region sustains `min(pods, 64) * W` concurrent checks; sized so a region's steady job rate is comfortably below its drain rate, with the HPA absorbing bursts.
...
| Control-plane Postgres unavailable | the worker does not write the durable `check_results` row, so checking and emitting are unaffected (its side-table writes may fail and retry, but the check still runs); the persist (folded into alerting in the control plane) backs up on the bus (`check.results` 24h retention) and catches up on recovery | the data plane keeps checking and emitting; the durable write is delayed, not lost; this is exactly why Option B was chosen over a synchronous cross-region write |
