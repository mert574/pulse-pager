...
| Domain and status subdomain | `pulse.app`, `{slug}.pulse.app` | Recommended to keep for now. A domain move (for example to `pulsepager.app`) is a real infra and DNS and TLS and email-deliverability project: redirects, cert reissue, OAuth callback URLs, status-page subdomain cutover, SEO. PO decision: track it as a **separate future infra task**, explicitly **not part of this rename**. The display name being "Pulse Pager" while the URL is still `pulse.app` is fine and common; users read the name, they rarely read the registrable domain. |
