...
const planPriceColumns = `id, provider, plan, cycle, provider_price_id, trial_days,
...
func scanPlanPrice(row pgx.Row) (*domain.PlanPrice, error) {
	...
	err := row.Scan(&p.ID, &p.Provider, &p.Plan, &p.Cycle, &p.ProviderPriceID,
		&p.TrialDays, &p.CustomData, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &p, nil
...
func (p *Pool) UpsertPlanPrice(ctx context.Context, pp *domain.PlanPrice) error {
	...
	_, err := p.Exec(ctx, `
		INSERT INTO plan_prices (provider, plan, cycle, provider_price_id, trial_days, custom_data, updated_at)
		VALUES ($1,$2,$3,$4,$5,$6, now())
		ON CONFLICT (provider, plan, cycle) DO UPDATE SET
			provider_price_id = EXCLUDED.provider_price_id,
			trial_days = EXCLUDED.trial_days,
			custom_data = EXCLUDED.custom_data,
			updated_at = now()`,
		pp.Provider, pp.Plan, pp.Cycle, pp.ProviderPriceID, pp.TrialDays, cd)
	return err
}
