...
// Inline icon set, paths derived from lucide (https://lucide.dev, ISC). Authored
// inline rather than imported from the lucide package so it works the same in
// Vite and the esbuild test runner (no `.svg?raw` loader needed) and only the
// icons we use ship. Stroke icons inherit currentColor; size via a class.
...
const PATHS: Record<string, TemplateResult> = {
  ...
  incident: svg`<path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/>`,
  ...
  billing: svg`<rect width="20" height="14" x="2" y="5" rx="2"/><line x1="2" x2="22" y1="10" y2="10"/>`,
  ...
  account: svg`<circle cx="12" cy="12" r="10"/><circle cx="12" cy="10" r="3"/><path d="M7 20.662V19a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v1.662"/>`,
...
// icon(name) returns an SVG sized by the class (default size-4). Decorative by
// default (aria-hidden); pass a label for a meaningful standalone icon.
export function icon(name: IconName, cls = "size-4", label?: string) {
  return html`<svg
    class=${cls}
    viewBox="0 0 24 24"
    fill="none"
